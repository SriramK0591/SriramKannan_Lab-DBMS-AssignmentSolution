...
SELECT B.CUS_GENDER AS GENDER, COUNT(DISTINCT A.CUS_ID) AS TOTAL_CUSTOMERS FROM ORDERS A LEFT JOIN CUSTOMER B ON A.CUS_ID=B.CUS_ID 
WHERE A.ORD_AMOUNT>=3000
GROUP BY B.CUS_GENDER

-- 4) Display all the orders along with product name ordered by a customer having Customer_Id=2
SELECT D.CUS_NAME AS CUSTOMER_NAME, Q.PRODUCT AS PRODUCT FROM (SELECT P.*, C.PRO_NAME AS PRODUCT FROM
...
SELECT A.SUPP_ID AS SUPPLIER_ID, B.SUPP_NAME AS SUPPLIER_NAME, B.SUPP_CITY AS SUPPLIER_CITY, B.SUPP_PHONE AS SUPPLIER_PHONE
FROM
...
SELECT R.CAT_ID AS CATEGORY_ID, R.CAT_NAME AS CATEGORY_NAME, R.PRO_ID AS PRODUCT_ID, R.PRO_NAME AS PRODUCT_NAME FROM 
(SELECT Q.*, ROW_NUMBER() OVER (PARTITION BY Q.CAT_ID ORDER BY Q.SUPP_PRICE ASC) AS RNK
...
SELECT P.PRO_ID AS PRODUCT_ID, C.PRO_NAME AS PRODUCT FROM
...
<<<<<<< HEAD
SELECT DISTINCT CUS_NAME AS CUSTOMER_NAME, CUS_GENDER AS GENDER FROM CUSTOMER WHERE CUS_NAME LIKE 'A%' OR CUS_NAME LIKE '%A'
=======
SELECT DISTINCT CUS_NAME, CUS_GENDER FROM CUSTOMER WHERE CUS_NAME LIKE 'A%' OR CUS_NAME LIKE '%A'
>>>>>>> 70dfc96439db2be3b37626fd4e2f3b83793b1795
...
BEGIN
SELECT E.SUPP_ID AS SUPPLIER_ID, F.SUPP_NAME AS SUPPLIER_NAME, E.RAT_RATSTARS AS RATING, 
